...
async function prepareModulePhoto(file) {
...
function moduleUnitPage(typeId, unitId) {
  ...
  const calculatedProgress = unit.stages.length ? Math.round(unit.stages.reduce((sum, stage) => sum + Math.max(0, Math.min(100, Number(stage.percent) || 0)), 0) / unit.stages.length) : 0
  unit.progress = calculatedProgress
  content.innerHTML = `<section class="page-heading module-detail-heading"><div><button class="back-link" id="back-module-type">&larr; Nazad na ${type.label}</button><p class="eyebrow">Detalji modula</p><h1>${unit.id}</h1><p>Vodite evidenciju radova i fotografija za ovaj modul.</p></div><div class="module-total"><span>Napredak</span><b>${unit.progress}%</b></div></section><section class="unit-detail-grid"><article class="plan-panel"><h2>Stanje radova</h2><form id="module-unit-form" class="unit-form"><label>Ukupni procenat zavrsenosti<input name="progress" type="number" min="0" max="100" value="${unit.progress}" readonly title="Automatski zbir procenata svih faza"></label><label>Sta je uradjeno<input name="work" maxlength="180" value="${esc(unit.work)}" placeholder="npr. Postavljen plywood i cetris"></label><label>Beleška<textarea name="note" maxlength="1000" placeholder="Upisite napomenu za ovaj modul...">${esc(unit.note)}</textarea></label><button class="primary-btn">Sacuvaj promene</button></form></article><article class="plan-panel gallery-panel"><header><div><h2>Fotografije modula</h2><p>${today} · Fotografije sa kamere ili galerije tableta.</p></div></header><label class="photo-upload"><span>+ Dodaj fotografiju</span><input id="module-photo-input" type="file" accept="image/*" capture="environment" multiple></label><p class="photo-help">Fotografije se prilagođavaju i čuvaju u aplikaciji na ovom tabletu. Nema ograničenja na format fotografije niti na tri slike.</p><div id="module-gallery" class="module-gallery">${unit.photos.length ? unit.photos.map((photo, index) => `<figure><img src="${photo.data}" alt="Fotografija ${index + 1}"><figcaption>${photo.date}<button data-delete-photo="${index}" title="Obrisi fotografiju">&times;</button></figcaption></figure>`).join('') : '<p class="plan-empty">Jos nema fotografija za ovaj modul.</p>'}</div></article></section>`
  const doneStages = unit.stages.filter((stage) => stage.status === 'zavrseno').length
  content.insertAdjacentHTML('beforeend', `<section class="plan-panel stages-panel"><header><div><h2>Faze rada modula</h2><p>Za svaku fazu oznacite trenutno stanje rada ili promenite redosled.</p></div><span>${doneStages}/${unit.stages.length} zavrseno</span></header><div id="module-stages" class="module-stages">${unit.stages.map((stage, index) => { const status = moduleStageStatus[stage.status] || moduleStageStatus['nije-zavrseno']; return `<article class="module-stage"><div class="stage-title"><span>${index + 1}</span><b>${esc(stage.title)}</b></div><div class="stage-status-actions"><button class="${stage.status === 'nije-zavrseno' ? 'selected ' : ''}stage-red" data-stage-status="nije-zavrseno" data-stage-id="${stage.id}" title="Nije zavrseno">&times;</button><button class="${stage.status === 'u-toku' ? 'selected ' : ''}stage-yellow" data-stage-status="u-toku" data-stage-id="${stage.id}" title="U toku">&#8226;</button><button class="${stage.status === 'zavrseno' ? 'selected ' : ''}stage-green" data-stage-status="zavrseno" data-stage-id="${stage.id}" title="Zavrseno">&#10003;</button><button class="stage-edit" data-edit-stage="${stage.id}" title="Izmeni naziv faze">&#9998;</button><button class="stage-move" data-move-stage="up" data-stage-id="${stage.id}" title="Pomeri gore" ${index === 0 ? 'disabled' : ''}>&uarr;</button><button class="stage-move" data-move-stage="down" data-stage-id="${stage.id}" title="Pomeri dole" ${index === unit.stages.length - 1 ? 'disabled' : ''}>&darr;</button><button class="stage-remove" data-remove-stage="${stage.id}" title="Obrisi fazu">&times;</button></div><div class="stage-status-line"><small class="${status.className}">${status.label}</small><label class="stage-percent"><span>Procenat</span><input data-stage-percent="${stage.id}" type="number" min="0" max="100" value="${Number(stage.percent) > 0 ? Math.max(0, Math.min(100, Number(stage.percent))) : ''}" placeholder="—" title="Procenat zavrsenosti"><i>%</i></label></div></article>` }).join('')}</div><form id="module-stage-form" class="stage-add-form"><input name="stage-title" maxlength="120" placeholder="Dodajte novu fazu rada..." required><select name="insert-after"><option value="start">Dodaj na pocetak</option>${unit.stages.map((stage, index) => `<option value="${stage.id}" ${index === unit.stages.length - 1 ? 'selected' : ''}>Dodaj iza: ${esc(stage.title)}</option>`).join('')}</select><button class="primary-btn">+ Dodaj fazu</button></form></section>`)
  content.insertAdjacentHTML('beforeend', `<section class="plan-panel consumption-panel"><header><div><h2>Potrosnja materijala</h2><p>Materijal se automatski skida sa stanja magacina.</p></div><span>${unit.consumption.length} stavki</span></header><div class="consumption-stock-tools"><label><span>⌕</span><input id="consumption-material-search" type="search" placeholder="Pretraži sav materijal iz skladišta..."></label><b>${materials.length} artikala</b></div><div id="consumption-stock-list" class="consumption-stock-list">${materials.length ? materials.map((item) => `<article data-consumption-material="${item.id}" data-consumption-search="${esc(`${item.name} ${item.standard || ''} ${item.location || ''}`).toLocaleLowerCase('sr')}"><div><b>${esc(item.name)}</b><small>${esc(item.standard || 'Bez standarda')} · ${esc(item.location || 'Lokacija nije unesena')}</small></div><strong>${new Intl.NumberFormat('sr-RS').format(item.stock)} ${esc(item.unit)}</strong><label><span>Količina</span><input data-consumption-quantity type="number" min="1" max="${Math.max(1, Number(item.stock) || 1)}" value="1" inputmode="numeric"></label><button type="button" data-consumption-take="${item.id}" ${Number(item.stock) <= 0 ? 'disabled' : ''}>Skini</button></article>`).join('') : '<p class="plan-empty">Nema materijala u skladištu. Prvo ga dodajte u kartici Materijal.</p>'}</div><div id="consumption-list" class="consumption-list">${unit.consumption.length ? unit.consumption.slice().reverse().map((entry) => `<article><div><b>${esc(entry.name)}</b><p>${entry.date} &middot; ${entry.quantity} ${esc(entry.unit)} skinuto sa stanja</p></div><strong>-${entry.quantity} ${esc(entry.unit)}</strong><button data-undo-consumption="${entry.id}" title="Vrati materijal na stanje">&crarr;</button></article>`).join('') : '<p class="plan-empty">Jos nije evidentirana potrosnja materijala za ovaj modul.</p>'}</div></section>`)
  ...
  content.insertAdjacentHTML('beforeend', `<style>.consumption-stock-tools{display:flex;align-items:center;justify-content:space-between;gap:14px;margin-bottom:12px}.consumption-stock-tools label{display:flex;align-items:center;gap:9px;flex:1;max-width:680px;padding:0 12px;border:1px solid #365b7b;border-radius:10px;background:#102039}.consumption-stock-tools label span{color:#65d9ff;font-size:18px}.consumption-stock-tools input{width:100%;height:43px;border:0;outline:0;background:transparent;color:#f0f8ff}.consumption-stock-tools>b{color:#72dcff;font;font-size:11px}.consumption-stock-list{overflow:hidden;border:1px solid #2d4b68;border-radius:12px}.consumption-stock-list>article{display:grid;grid-template-columns:minmax(230px,1fr) 150px 120px 100px;gap:14px;align-items:center;padding:12px 14px;border-bottom:1px solid #294762;background:rgba(12,29,49,.28)}.consumption-stock-list>article:last-child{border-bottom:0}.consumption-stock-list>article[hidden]{display:none!important}.consumption-stock-list>article>div{display:grid;gap:4px}.consumption-stock-list>article>div>b{font-size:13px}.consumption-stock-list>article>div>small{color:#8fa9c2;font-size:10px}.consumption-stock-list>article>strong{color:#7ee7ff;font-size:13px;text-align:right}.consumption-stock-list>article>label{display:flex;align-items:center;gap:7px}.consumption-stock-list>article>label span{color:#91a9c0;font-size:9px;font-weight:800}.consumption-stock-list input{width:62px;height:35px;padding:0 7px;border:1px solid #3d6686;border-radius:8px;outline:0;background:#102039;color:#fff;text-align:center;font-weight:800}.consumption-stock-list button{height:37px;border:1px solid #43cbf6;border-radius:9px;background:#43bfee;color:#071526;font-weight:900;cursor:pointer}.consumption-stock-list button:hover{filter:brightness(1.1);box-shadow:0 0 15px rgba(67,203,246,.3)}.consumption-stock-list button:disabled{border-color:#344c62;background:#26384b;color:#71869b;cursor:not-allowed;box-shadow:none}@media(max-width:850px){.consumption-stock-list>article{grid-template-columns:1fr auto}.consumption-stock-list>article>strong{text-align:left}.consumption-stock-list>article>label,.consumption-stock-list>article>button{grid-row:2}.consumption-stock-tools{align-items:stretch;flex-direction:column}.consumption-stock-tools label{max-width:none}}</style>`)
  ...
  document.querySelector('#module-stage-form').addEventListener('submit', (event) => {
    ...
    if (!title) return
    const newStage = { id: `dodatno-${Date.now()}`, title, status: 'nije-zavrseno', percent: null, custom: true }
  ...
  document.querySelector('#module-stages').addEventListener('change', (event) => {
    ...
    if (!stage) return
    stage.percent = input.value.trim() === '' ? null : Math.max(0, Math.min(100, Number(input.value) || 0))
    unit.progress = unit.stages.length ? Math.round(unit.stages.reduce((sum, item) => sum + Math.max(0, Math.min(100, Number(item.percent) || 0)), 0) / unit.stages.length) : 0
    saveModuleDetails()
    moduleUnitPage(typeId, unitId)
  })
  document.querySelector('#module-unit-form').addEventListener('submit', (event) => { event.preventDefault(); const form = new FormData(event.currentTarget); unit.progress = unit.stages.length ? Math.round(unit.stages.reduce((sum, stage) => sum + Math.max(0, Math.min(100, Number(stage.percent) || 0)), 0) / unit.stages.length) : 0; unit.work = form.get('work').trim(); unit.note = form.get('note').trim(); saveModuleDetails(); moduleUnitPage(typeId, unitId) })
  ...
  document.querySelector('#module-photo-input').addEventListener('change', async (event) => {
    const files = [...event.target.files]
    if (!files.length) return
    try {
      const photos = []
      for (const file of files) photos.push({ data: await prepareModulePhoto(file), date: today, name: file.name || 'Fotografija' })
      unit.photos.push(...photos)
      saveModuleDetails()
      moduleUnitPage(typeId, unitId)
    } catch (error) {
      alert('Fotografija nije mogla biti sačuvana. Provjerite slobodan prostor na tabletu i pokušajte ponovo.')
    }
  })
  document.querySelector('#consumption-material-search')?.addEventListener('input', (event) => {
    const term = event.target.value.trim().toLocaleLowerCase('sr')
    document.querySelectorAll('[data-consumption-material]').forEach((row) => { row.hidden = term && !row.dataset.consumptionSearch.includes(term) })
  })
  document.querySelector('#consumption-stock-list')?.addEventListener('click', (event) => {
    const button = event.target.closest('[data-consumption-take]')
    if (!button) return
    const row = button.closest('[data-consumption-material]')
    const item = materials.find((entry) => String(entry.id) === String(button.dataset.consumptionTake))
    const quantity = Number(row?.querySelector('[data-consumption-quantity]')?.value)
    if (!item || !Number.isFinite(quantity) || quantity <= 0) { alert('Unesite ispravnu količinu.'); return }
    if (Number(item.stock) < quantity) { alert(`Nema dovoljno materijala. Na stanju je ${item.stock} ${item.unit}.`); return }
    const before = Number(item.stock) || 0
    item.stock = before - quantity
    unit.consumption.push({ id: `${Date.now()}-${Math.random().toString(16).slice(2)}`, materialId: item.id, name: item.name, unit: item.unit, quantity, date: today, before, after: item.stock })
       saveInventory()
    saveModuleDetails()
    moduleUnitPage(typeId, unitId)
  })
